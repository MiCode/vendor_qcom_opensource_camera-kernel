...
static uint  debug_grab_ftrace = 0;
module_param(debug_grab_ftrace, uint, 0644);
pid_t g_CameraPid = 0;
EXPORT_SYMBOL(g_CameraPid);
...
static int cam_req_mgr_open(struct file *filep)
{
	...
	if (debug_grab_ftrace) {
		g_CameraPid = task_pid_nr(current);
		CAM_INFO(CAM_CRM, "[DEBUG_DUMP_FTRACE] get current task:%d", g_CameraPid);
	}
