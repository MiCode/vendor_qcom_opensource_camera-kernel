...
static int check_concurrency = 0;
...
{
	int idx = 0;
	int next_frame_timeout, current_frame_timeout, applied_frame_timeout, max_frame_timeout;
	...
	applied_frame_timeout = in_q->slot[in_q->rd_idx].additional_timeout;
	CAM_DBG(CAM_CRM,
		"rd_idx: %d applied_req_id, applied_frame_timeout: %d ms",
		in_q->rd_idx, in_q->slot[in_q->rd_idx].req_id,
		applied_frame_timeout);
	...
	max_frame_timeout = (applied_frame_timeout > max_frame_timeout) ?
		applied_frame_timeout : max_frame_timeout;
	...
	struct cam_req_mgr_core_link *link)
{
	...
	if ((slot->status == CRM_SLOT_STATUS_REQ_APPLIED) ||
	    (slot->status == CRM_SLOT_STATUS_REQ_DONE)) {
		CAM_WARN(CAM_CRM,
...
//xiaomi add end
	...
	CAM_DBG(CAM_CRM, "begin checking concurrent for req : %lld", apply_data[CAM_PIPELINE_DELAY_1].req_id);
	if ((apply_data[CAM_PIPELINE_DELAY_1].req_id > 1) && check_concurrency) {
		j = 0;
		k = 0;
		mutex_lock(&session->lock);
		for (i = 0; i < MAXIMUM_LINKS_PER_SESSION; i++) {
			sync_link = session->links[i];
			CAM_DBG(CAM_CRM, "checking %d/%d sync_link:%p link:%p", i, MAXIMUM_LINKS_PER_SESSION, sync_link, link);
			if (sync_link && sync_link != link) {
				sync_slot_idx = __cam_req_mgr_find_slot_for_req(
					sync_link->req.in_q,
					apply_data[CAM_PIPELINE_DELAY_1].req_id);
				CAM_DBG(CAM_CRM, "sync link:%x req:%lld slot_idx:%d",
					sync_link->link_hdl, apply_data[CAM_PIPELINE_DELAY_1].req_id,
					sync_slot_idx);
				if (sync_slot_idx > 0)
					j++;
		...
		if (j >= 3)
			CAM_WARN(CAM_CRM, "%d concurrent cameras have req#%lld",
				j, apply_data[CAM_PIPELINE_DELAY_1].req_id);
		else
			CAM_DBG(CAM_CRM, "%d concurrent cameras have req#%lld",
				j, apply_data[CAM_PIPELINE_DELAY_1].req_id);

		slot_idx = __cam_req_mgr_find_slot_for_req(link->req.in_q,
			apply_data[CAM_PIPELINE_DELAY_1].req_id - 1);

		CAM_DBG(CAM_CRM, "link:%x slot_idx:%d prev has req:%s",
			link->link_hdl, slot_idx, (slot_idx == -1) ? "NO" : "YES");

		/* No previous req and there are more than one other links */
		if ((slot_idx == -1) && (k > 1)) {
			j = 0;
			for (i = 0; i < MAXIMUM_LINKS_PER_SESSION; i++) {
				sync_link = session->links[i];
				if (sync_link && sync_link != link) {
					...
					CAM_DBG(CAM_CRM, "sync_link:%x find req:%lld sync_slot_idx:%d", sync_link->link_hdl,
						apply_data[CAM_PIPELINE_DELAY_1].req_id - 1, sync_slot_idx);

					if (sync_slot_idx >= 0) {
						cam_common_inc_idx(&sync_slot_idx, 1, sync_link->req.in_q->num_slots);
						sync_slot = &sync_link->req.in_q->slot[sync_slot_idx];

						if (sync_slot->status != CRM_SLOT_STATUS_REQ_DONE) {
							j++;
							CAM_DBG(CAM_CRM,
								"sync_link:%x slot:%x req:%lld hasn't done, status:%d",
								sync_link->link_hdl, sync_slot->idx, sync_slot->req_id, sync_slot->status);
						} else {
							CAM_DBG(CAM_CRM, "sync_link:%x all previous req has done",
								sync_link->link_hdl);
						}
					}
				}
			...
			/* Check previous req count in sync link */
			CAM_DBG(CAM_CRM, "link:%x sync prev_req:%lld hasn't done, count is %d", link->link_hdl,
				apply_data[CAM_PIPELINE_DELAY_1].req_id - 1, j);
			if (j >= 1) {
				CAM_INFO(CAM_CRM, "link:%x drop current frame for req:%lld due to concurrency",
					link->link_hdl, apply_data[CAM_PIPELINE_DELAY_1].req_id - 1);
	...
	struct cam_req_mgr_trigger_notify *trigger_data)
{
	...
	rc = __cam_req_mgr_send_req(link, link->req.in_q, trigger, trigger_data->frame_id, &dev);
	if (rc < 0) {
...
int cam_req_mgr_process_sched_req(void *priv, void *data)
{
	...
	prev_wr_idx = in_q->wr_idx;
	cam_common_dec_idx(&prev_wr_idx, 1, in_q->num_slots);
	slot = &in_q->slot[prev_wr_idx];
	CAM_DBG(CAM_CRM,
		"link_hdl %x prev_wr_idx:%d curr_req:%lld prev_req:%lld",
		sched_req->link_hdl, prev_wr_idx,
		sched_req->req_id, slot->req_id);
	if ((slot->req_id > 1) && (sched_req->req_id > (slot->req_id + 1)) && check_concurrency &&
		!(CAM_LINK_PROPERTY_AUX_SESSION & link->properties_mask)) {
		session = (struct cam_req_mgr_core_session *)link->parent;
		if (!session) {
			CAM_WARN(CAM_CRM, "session ptr NULL %x", link->link_hdl);
			mutex_unlock(&link->req.lock);
		...
		for (i = 0; i < MAXIMUM_LINKS_PER_SESSION; i++) {
			sync_link = session->links[i];
			if (sync_link && sync_link != link &&
				!(CAM_LINK_PROPERTY_AUX_SESSION & sync_link->properties_mask)) {
				sync_slot_idx = __cam_req_mgr_find_slot_for_req(
					sync_link->req.in_q,
					sched_req->req_id - 1);
				CAM_DBG(CAM_CRM, "sync link:%x req:%lld slot_idx:%d",
		...
		CAM_DBG(CAM_CRM, "link_hdl %x mask:0x%x", sched_req->link_hdl, link->properties_mask);
		if (j > 0) {
			cam_common_inc_idx(&in_q->wr_idx, 1, in_q->num_slots);
			CAM_INFO(CAM_CRM,
				"Insert an empty slot for link_hdl %x, req_id %lld will be at slot %d",
				sched_req->link_hdl, sched_req->req_id,
				in_q->wr_idx);
		} else {
			CAM_DBG(CAM_CRM, "link_hdl %x, req_id %lld no need to insert an empty slot",
				sched_req->link_hdl, sched_req->req_id);
		}
	}
...
static int cam_req_mgr_process_trigger(void *priv, void *data)
{
	...
	if ((trigger_data->trigger == CAM_TRIGGER_POINT_SOF) &&
		...
		(in_q->slot[in_q->rd_idx].status == CRM_SLOT_STATUS_REQ_DONE))) {
		/*
	...
	struct cam_req_mgr_notify_msg *msg)
{
	...
	case CAM_REQ_MGR_MSG_UPDATE_DEVICE_INFO: {
		...
		break;
	case CAM_REQ_MGR_MSG_NOTIFY_REQ_DONE: {
		spin_lock_bh(&link->req.reset_link_spin_lock);
		in_q = link->req.in_q;
		if (!in_q) {
			CAM_ERR(CAM_CRM, "in_q ptr NULL, link_hdl %x", msg->link_hdl);
		...
		cam_common_inc_idx(&slot_idx, link->max_delay - 1, link->req.in_q->num_slots);
		slot = &in_q->slot[slot_idx];
		spin_unlock_bh(&link->req.reset_link_spin_lock);

		if (slot->status == CRM_SLOT_STATUS_REQ_APPLIED) {
			slot->status = CRM_SLOT_STATUS_REQ_DONE;
			CAM_DBG(CAM_CRM, "req:%lld slot:%d status change from %d to %d on link:0x%x",
				msg->req_id, slot_idx, CRM_SLOT_STATUS_REQ_APPLIED,
				CRM_SLOT_STATUS_REQ_DONE, link->link_hdl);
		} else {
			rc = -EINVAL;
			CAM_ERR(CAM_CRM, "Req:%lld status isn't APPLIED on link: 0x%x",
				msg->req_id, link->link_hdl);
		}
	}
		break;
	...
	struct cam_req_mgr_session_info *ses_info)
{
	...
	check_concurrency = ses_info->is_check_concurrency; //xiaomi added
end:
