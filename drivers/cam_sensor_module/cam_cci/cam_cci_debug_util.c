	...
	enum camera_sensor_i2c_type data_type)
{
	...
	rc = camera_io_dev_read_seq(io_master_info, start, data,
	...
	char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	snprintf(buf + buf_pos, count - buf_pos, "--- %.*llu:%.*llu:%.*llu.%.*llu\n",
	...
	void **cci_debug)
{
	...
	if (io_master_info->cci_client) {
		...
		dbgfileptr = debugfs_lookup(debugfs_name, dbgfileptr);
		if (!dbgfileptr) {
			CAM_ERR(MI_DEBUG, "%s: qup master debugfs dir lookup failed");
			rc = -ENOENT;
			goto end;
		}
		cci_dbg->parent_entry = dbgfileptr;
		...
		dbgfileptr = debugfs_create_dir(debugfs_name, dbgfileptr);
		if (!dbgfileptr) {
			CAM_ERR(MI_DEBUG, "%s: qup device debugfs dir creation fail",
				dev_type);
			rc = -ENOENT;
			goto end;
		}
	} else {
		CAM_WARN(MI_DEBUG, "%s: invaild i2c devices type debugfs dir creation fail",
			dev_type);
		rc = -ENOENT;
		goto end;
	}
