...
static int otp_read_vendorid(struct cam_eeprom_ctrl_t *e_ctrl){

	int vendorid;
	init_otp_read_byte_gc08a8(e_ctrl);
	otp_set_addr_gc08a8(e_ctrl, 0x15a8);
	otp_byte_flag_gc08a8(e_ctrl);
	vendorid = otp_read_reg_gc08a8(e_ctrl);
	if (vendorid == 0x09)
		return 0x09;
	if (vendorid == 0x10)
		return 0x10;
	init_otp_read_byte_gc08a8(e_ctrl);
	otp_set_addr_gc08a8(e_ctrl, 0x8ab8);
	otp_byte_flag_gc08a8(e_ctrl);
	vendorid = otp_read_reg_gc08a8(e_ctrl);
	if (vendorid == 0x09)
		return 0x09;
	init_otp_read_byte_gc08a8(e_ctrl);
	otp_set_addr_gc08a8(e_ctrl, 0x8ac0);
	otp_byte_flag_gc08a8(e_ctrl);
	vendorid = otp_read_reg_gc08a8(e_ctrl);
	if (vendorid == 0x10)
		return 0x10;

	CAM_ERR(CAM_EEPROM, "read vendorid failed");
	return 0;
}
