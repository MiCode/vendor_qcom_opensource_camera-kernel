	...
	struct cam_eeprom_memory_block_t *block)
{
	...
	for (j = 0; j < block->num_map; j++) {
		...
		camera_io_dev_read(&e_ctrl->io_master_info, OTP_OV08F10_SENSORID_REG_H, &sensorid_h, CAMERA_SENSOR_I2C_TYPE_BYTE, CAMERA_SENSOR_I2C_TYPE_BYTE);
		camera_io_dev_read(&e_ctrl->io_master_info, OTP_OV08F10_SENSORID_REG_L, &sensorid_l, CAMERA_SENSOR_I2C_TYPE_BYTE, CAMERA_SENSOR_I2C_TYPE_BYTE);
		CAM_INFO(CAM_EEPROM, "sensorid = 0x%x", sensorid);
		CAM_INFO(CAM_EEPROM, "sensor_id0 = 0x%x", sensorid_h);
		CAM_INFO(CAM_EEPROM, "sensor_id1 = 0x%x", sensorid_l);
		if ((sensorid_h == 0x56)&&(sensorid_l == 0x08))  //20250329 sensor check
		{
			CAM_INFO(CAM_EEPROM, "OV08F10 otp init start");
			init_otp_read_byte_ov08f10(e_ctrl);

			otp_set_page_ov08f10(e_ctrl, 0x04);
			camera_io_dev_read(&e_ctrl->io_master_info, 0x00, &otp_flag_0, CAMERA_SENSOR_I2C_TYPE_BYTE, CAMERA_SENSOR_I2C_TYPE_BYTE);
			camera_io_dev_read(&e_ctrl->io_master_info, 0x01, &otp_flag_1, CAMERA_SENSOR_I2C_TYPE_BYTE, CAMERA_SENSOR_I2C_TYPE_BYTE);
			camera_io_dev_read(&e_ctrl->io_master_info, 0x02, &otp_flag_2, CAMERA_SENSOR_I2C_TYPE_BYTE, CAMERA_SENSOR_I2C_TYPE_BYTE);
			camera_io_dev_read(&e_ctrl->io_master_info, 0x03, &otp_flag_3, CAMERA_SENSOR_I2C_TYPE_BYTE, CAMERA_SENSOR_I2C_TYPE_BYTE);

			CAM_INFO(CAM_EEPROM, "OV08F10 otp_flag0 = 0x%x", otp_flag_0);
			CAM_INFO(CAM_EEPROM, "OV08F10 otp_flag1 = 0x%x", otp_flag_1);
			CAM_INFO(CAM_EEPROM, "OV08F10 otp_flag2 = 0x%x", otp_flag_2);
			CAM_INFO(CAM_EEPROM, "OV08F10 otp_flag3 = 0x%x", otp_flag_3);

			//memptr[i] = otp_flag;
			if ((otp_flag_2 == 0xFF) && (otp_flag_3 == 0xFF))
			{
				for(page = 0x22; page <= 0x3f; page++)
				{
					otp_set_page_ov08f10(e_ctrl, page);
					if(page == 0x22)
					{
						camera_io_dev_read_seq(&e_ctrl->io_master_info, 0x02, memptr, CAMERA_SENSOR_I2C_TYPE_BYTE, CAMERA_SENSOR_I2C_TYPE_BYTE, 0x80-0x02);
						memptr += (0x80-0x02);
					}
					else
					{
						camera_io_dev_read_seq(&e_ctrl->io_master_info, 0x00, memptr, CAMERA_SENSOR_I2C_TYPE_BYTE, CAMERA_SENSOR_I2C_TYPE_BYTE, 0x80);
						memptr += 0x80;
					}
				}
				//memptr += emap[j].mem.valid_size;
			}
			else if ((otp_flag_0 == 0xFF) && (otp_flag_1 == 0xFF) && (otp_flag_2 == 0x00) && (otp_flag_3 == 0x00))
			{
				for(page = 0x04; page <= 0x22; page++)
				{
					otp_set_page_ov08f10(e_ctrl, page);
					if(page == 0x04)
					{
						camera_io_dev_read_seq(&e_ctrl->io_master_info, 0x04, memptr, CAMERA_SENSOR_I2C_TYPE_BYTE, CAMERA_SENSOR_I2C_TYPE_BYTE, 0x80-0x04);
						memptr += (0x80-0x04);
					}
					else if(page == 0x22)
					{
						camera_io_dev_read_seq(&e_ctrl->io_master_info, 0x00, memptr, CAMERA_SENSOR_I2C_TYPE_BYTE, CAMERA_SENSOR_I2C_TYPE_BYTE, 0x2);
						memptr += 0x2;
					}
					else
					{
			...
			{
				CAM_ERR(CAM_EEPROM, "OV08F10 read group_flag error");
			}
			CAM_INFO(CAM_EEPROM, "OV08F10 otp init end, size = %d", i);
		}
		else if (sensorid == 0x08a8)
		{
			CAM_INFO(CAM_EEPROM, "gc08a8 otp init start");
			init_otp_read_byte_gc08a8(e_ctrl);
			otp_set_addr_gc08a8(e_ctrl, 0xfef0);
			otp_byte_flag_gc08a8(e_ctrl);
			otp_flag = otp_read_reg_gc08a8(e_ctrl);
			init_otp_read_byte_gc08a8(e_ctrl);
			otp_set_addr_gc08a8(e_ctrl, 0x89d8);
			otp_byte_flag_gc08a8(e_ctrl);
			otp_flag2 = otp_read_reg_gc08a8(e_ctrl);
			//memptr[i] = otp_flag;
			CAM_INFO(CAM_EEPROM, "gc08a8 otp group_flag = %d", otp_flag);
			if (otp_flag == 0x55)
			{
				init_otp_read_continue_gc08a8(e_ctrl);
				otp_set_addr_gc08a8(e_ctrl, 0x14c8);
				...
				for (addr = start_addr; addr <= end_addr; addr = addr + 8)
				{
					...
					memptr[i] = otp_data;
					++i;
					//CAM_ERR(CAM_EEPROM, "group 1 otp_data 0x%4x = 0x%2x", addr, otp_data);
					//CAM_ERR(CAM_EEPROM, "group 1 memptr[%d] = 0x%2x", i, memptr[i]);
				}
				memptr += emap[j].mem.valid_size;
			}
			else if (otp_flag2 == 0x01)
			{
				init_otp_read_continue_gc08a8(e_ctrl);
				otp_set_addr_gc08a8(e_ctrl, 0x89e0);
				otp_continue_flag_gc08a8(e_ctrl);
				...
				for (addr = start_addr; addr <= end_addr; addr = addr + 8)
				{
					...
					memptr[i] = otp_data;
					++i;
					//CAM_ERR(CAM_EEPROM, "group 2 otp_data 0x%4x = 0x%2x", addr, otp_data);
					//CAM_ERR(CAM_EEPROM, "group 2 memptr[%d] = 0x%2x", i, memptr[i]);
				}
				memptr += emap[j].mem.valid_size;
			}
			else
			{
				CAM_ERR(CAM_EEPROM, "GC08A8 read group_flag error");
			}
			CAM_INFO(CAM_EEPROM, "gc08a8 otp init end");
		}
		else{
