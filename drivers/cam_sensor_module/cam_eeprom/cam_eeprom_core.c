...
//			i2c_reg_array.delay = emap[j].page.delay;
			if(CAMERA_SENSOR_WAIT_OP_SW_UCND == emap[j].page.delay_type) {
			...
			CAM_DBG(CAM_EEPROM, "emap[%d] HWdelay = %d, SWdelay = %d", j, i2c_reg_array.delay, i2c_reg_settings.delay);
			/* xiaomi add */
	...
	int *num_map, size_t remain_buf_len)
{
	...
	case CAMERA_SENSOR_CMD_TYPE_WAIT:
		if (generic_op_code ==
			...
			map[*num_map - 1].page.delay_type = generic_op_code; /* xiaomi add */
