	...
	struct cam_eeprom_memory_block_t *block)
{
	...
	for (j = 0; j < block->num_map; j++) {
		...
		if (emap[j].page.valid_size) {
			...
			if (emap[j].page.delay_code == CAMERA_SENSOR_WAIT_OP_SW_UCND) {
	...
	int *num_map, size_t remain_buf_len)
{
	...
	case CAMERA_SENSOR_CMD_TYPE_WAIT:
		if (generic_op_code ==
			...
			map[*num_map - 1].mem.delay_code = generic_op_code;
			map[*num_map - 1].page.delay = i2c_uncond_wait->delay;
			map[*num_map - 1].page.delay_code = generic_op_code;
			map[*num_map - 1].pageen.delay = i2c_uncond_wait->delay;
			map[*num_map - 1].pageen.delay_code = generic_op_code;
