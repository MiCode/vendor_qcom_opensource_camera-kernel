	...
	struct cam_eeprom_memory_block_t *block)
{
	...
	for (j = 0; j < block->num_map; j++) {
		...
		if (sensorid == 0x08a8)
		{
			...
			CAM_DBG(CAM_EEPROM, "zengx otp group_flag = %d", otp_flag);
			if (otp_flag == 0x00)
			{
				...
				for (addr = start_addr; addr <= end_addr; addr = addr + 8)
				{
					...
					memptr[i] = otp_data;
					++i;
			                CAM_DBG(CAM_EEPROM, "group 1 otp_data 0x%4x = 0x%2x", addr, otp_data);
			                CAM_DBG(CAM_EEPROM, "group 1 memptr[%d] = 0x%2x", i, memptr[i]);
				}
				otp_set_addr_gc08a8(e_ctrl, 0x8930);
				otp_continue_flag_gc08a8(e_ctrl);
				for (addr = 0x8930; addr <= 0x89B8; addr = addr + 8)
				{
					otp_data = otp_read_reg_gc08a8(e_ctrl);
					memptr[i] = otp_data;
					++i;
			...
			else if (otp_flag == 0x01)
			{
				...
				for (addr = start_addr; addr <= end_addr; addr = addr + 8)
				{
					...
					memptr[i] = otp_data;
					++i;
			                CAM_DBG(CAM_EEPROM, "group 2 otp_data 0x%4x = 0x%2x", addr, otp_data);
			                CAM_DBG(CAM_EEPROM, "group 2 memptr[%d] = 0x%2x", i, memptr[i]);
				}
				otp_set_addr_gc08a8(e_ctrl, 0xFE48);
				otp_continue_flag_gc08a8(e_ctrl);
				for (addr = 0x8930; addr <= 0x89B8; addr = addr + 8)
				{
					otp_data = otp_read_reg_gc08a8(e_ctrl);
			...
			CAM_DBG(CAM_EEPROM, "zengx otp init end");
		}
		else{
