...
struct xm_cam_dev_info* get_actuator_xm_cam_dev_info(struct cam_actuator_ctrl_t *a_ctrl)
{
	...
	return &(a_ctrl->xm_cam_dev_info_data);
}
/* xiaomi dev protection add*/
	...
	struct device *master_dev, void *data)
{
	...
	rc = cam_cci_dev_create_debugfs_entry(a_ctrl->device_name,
		...
		&a_ctrl->io_master_info, CCI_MASTER_MAX,
		&a_ctrl->cci_debug);
	if (rc) {
	...
	/* xiaomi dev protection add*/
	xm_cam_dev_init_dev_info(get_actuator_xm_cam_dev_info(a_ctrl), XM_CAM_DEV_TYPE_ACTUATOR);
	/* xiaomi dev protection add*/
	...
	struct device *master_dev, void *data)
{
	...
	a_ctrl->i2c_data.per_frame = NULL;
	/* xiaomi dev protection add*/
	xm_cam_dev_destroy_dev_info(get_actuator_xm_cam_dev_info(a_ctrl));
	/* xiaomi dev protection add*/
	...
	struct device *master_dev, void *data)
{
	...
	rc = cam_cci_dev_create_debugfs_entry(a_ctrl->device_name,
		a_ctrl->soc_info.index, CAM_ACTUATOR_NAME,
		&a_ctrl->io_master_info, a_ctrl->io_master_info.cci_client->cci_i2c_master,
		&a_ctrl->cci_debug);
	if (rc) {
		CAM_WARN(CAM_ACTUATOR, "debugfs creation failed");
		rc = 0;
