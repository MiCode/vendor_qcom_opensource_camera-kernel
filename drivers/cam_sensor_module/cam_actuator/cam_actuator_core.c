...
int32_t cam_actuator_apply_request(struct cam_req_mgr_apply_request *apply)
{
	...
	for (del_req_id = 0; del_req_id < MAX_PER_FRAME_ARRAY; del_req_id++) {
		if (apply->request_id >
			(a_ctrl->i2c_data.per_frame[del_req_id].request_id + MAX_SYSTEM_PIPELINE_DELAY)) {
			CAM_DBG(CAM_ACTUATOR, "Clean up per frame[%d] = %lld",
				del_req_id, a_ctrl->i2c_data.per_frame[del_req_id].request_id);
...
static int32_t parklens_thread_func(void *arg)
{
	...
	int32_t parklens_opcode = 0, rc = 0, j = 0;
	uint32_t size = 0;
		...
		if (i2c_set == NULL) {
			...
			if ((i2c_list_last != NULL) && list_entry_is_head(i2c_list, &(i2c_set->list_head), list)) {
				...
				switch (i2c_list->op_code) {
					...
					case CAM_SENSOR_I2C_WRITE_SEQ: {
						for (j = 0;j < i2c_list->i2c_settings.size;j++) {
							CAM_DBG(MI_PARKLENS,"actuator name %s Write, request id %d, j=[%d], reg_addr 0x%x, 0x%x", a_ctrl->device_name,
					...
					case CAM_SENSOR_I2C_POLL: {
						for (j = 0;j < i2c_list->i2c_settings.size;j++) {
							i2c_list->i2c_settings.delay = sleeptime;
							sleeptime = 0;
							CAM_DBG(MI_PARKLENS, "actuator name  %s POLL, request id %d, j=[%d], reg_addr 0x%x, 0x%x",a_ctrl->device_name,
								i2c_set->request_id, j,i2c_list->i2c_settings.reg_setting[j].reg_addr,
								i2c_list->i2c_settings.reg_setting[j].reg_data);
							trace_cam_i2c_write_log_event("[ACTUATORSETTINGS]", a_ctrl->device_name,
								i2c_set->request_id, j, "POLL", i2c_list->i2c_settings.reg_setting[j].reg_addr,
					...
					case CAM_SENSOR_I2C_READ_SEQ: {
						for (j = 0;j < i2c_list->i2c_settings.size;j++) {
							CAM_DBG(MI_PARKLENS, "actuator name  %s READ, request id %d, j=[%d], reg_addr 0x%x, 0x%x",a_ctrl->device_name,
								i2c_set->request_id, j,i2c_list->i2c_settings.reg_setting[j].reg_addr,
								i2c_list->i2c_settings.reg_setting[j].reg_data);
							trace_cam_i2c_write_log_event("[ACTUATORSETTINGS]", a_ctrl->device_name,
								i2c_set->request_id, j, "READ", i2c_list->i2c_settings.reg_setting[j].reg_addr,
