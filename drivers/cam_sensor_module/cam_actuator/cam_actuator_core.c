	...
	struct i2c_settings_array *i2c_set)
{
	...
	list_for_each_entry(i2c_list,
		...
		if (false == IS_DEV_IO_ERROR(a_ctrl->device_damage, a_ctrl->device_error)) {
			...
			if (rc < 0) {
				a_ctrl->device_error++;
				CAM_ERR(CAM_ACTUATOR,
					"Failed to apply settings: %d, IO fail count:%d", rc, a_ctrl->device_error);
				//add by xiaomi
	...
	void *arg)
{
	...
	case CAM_ACTUATOR_PACKET_OPCODE_INIT:
		...
		if (true == IS_DEV_IO_ERROR(a_ctrl->device_damage, a_ctrl->device_error)) {
			a_ctrl->init_error = true;
			rc = - ENODEV;
	...
	case CAM_ACTUATOR_PACKET_OPCODE_READ: {
		...
		if (false == IS_DEV_IO_ERROR(a_ctrl->device_damage, a_ctrl->device_error)) {
			...
			CAM_ERR(CAM_ACTUATOR, "damage count:%d IO fail count:%d skip read data!",
				a_ctrl->device_damage, a_ctrl->device_error);
		}
		...
		if (rc < 0 ||
			true == IS_DEV_IO_ERROR(a_ctrl->device_damage, a_ctrl->device_error)) {
			CAM_ERR(CAM_ACTUATOR,
	...
	void *arg)
{
	...
	case CAM_RELEASE_DEV: {
		...
		if (MAX_IO_FAIL_COUNT <= a_ctrl->device_error ||
				true == a_ctrl->init_error)
		{
			if (MAX_DAMAGE_COUNT > a_ctrl->device_damage)
			{
				a_ctrl->device_damage++;
				CAM_ERR(CAM_ACTUATOR, "device is damage count:%d", a_ctrl->device_damage);
			}
		}//If the number of damages has not been reached and no abnormalities have occurred, reset to zero
		else if (MAX_DAMAGE_COUNT > a_ctrl->device_damage){
			a_ctrl->device_damage = 0;
		}
		/* xiaomi modify end */
	}
		break;
