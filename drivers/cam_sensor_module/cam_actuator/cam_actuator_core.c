	...
	list_for_each_entry(i2c_list,
		...
		if (false == cam_check_cci_is_error(&a_ctrl->cci_err_info)) {
			...
			if (rc < 0) {
				...
				CAM_ERR(CAM_ACTUATOR,
					"Failed to apply settings: %d, IO fail count:%d",
				...
				CAM_DBG(CAM_ACTUATOR, "Success:request ID: %d, error count:%d",
					i2c_set->request_id,cam_cci_read_io_fail_count(&a_ctrl->cci_err_info));
			}
		}
		else {
			CAM_ERR(CAM_ACTUATOR, "damage count:%d IO fail count:%d skip apply setting!",
	...
	void *arg)
{
	...
	case CAM_ACTUATOR_PACKET_OPCODE_INIT:
		...
		if (true == cam_check_cci_is_error(&a_ctrl->cci_err_info)) {
			cam_cci_set_init_fail(&a_ctrl->cci_err_info);
			rc = - ENODEV;
			CAM_ERR(CAM_ACTUATOR, "device is damaged, rc:%d", rc);
		}
		break;
	...
	void *arg)
{
	...
	case CAM_ACQUIRE_DEV: {
		...
		cam_cci_reset_io_fail_count(&a_ctrl->cci_err_info);
		cam_check_cci_is_damage(&a_ctrl->cci_err_info, ActuatorI2C);
		a_ctrl->cam_act_state = CAM_ACTUATOR_ACQUIRE;
	}
		break;
	case CAM_RELEASE_DEV: {
		...
		if (true == cam_check_cci_is_damage(&a_ctrl->cci_err_info, ActuatorI2C))
			CAM_ERR(CAM_ACTUATOR, "dev may damaged!, damage count:%d",
				cam_cci_read_damage_count(&a_ctrl->cci_err_info));
		/* xiaomi modify end */
	}
		break;
		...
		if (i2c_set == NULL) {
			...
			if ((i2c_list_last != NULL) && list_entry_is_head(i2c_list, &(i2c_set->list_head), list)) {
				...
				if (false == cam_check_cci_is_error(&a_ctrl->cci_err_info)) {
					...
					rc = -ENODEV;
					CAM_ERR(MI_PARKLENS, "damage count:%d IO fail count:%d skip apply setting!",
						cam_cci_read_damage_count(&a_ctrl->cci_err_info),
						cam_cci_read_io_fail_count(&a_ctrl->cci_err_info));
				}
...
exit_without_powerdown:
	...
	if (NULL != parklens_wakelock.priv) {
