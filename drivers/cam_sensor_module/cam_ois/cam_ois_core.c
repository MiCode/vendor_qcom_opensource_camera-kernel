	...
	struct i2c_settings_array *i2c_set)
{
	...
	list_for_each_entry(i2c_list,
		...
		if (true == IS_DEV_IO_ERROR(o_ctrl->device_damage, o_ctrl->device_error)) {
			CAM_ERR(CAM_OIS, "damage count:%d IO fail count:%d skip apply setting!",
						o_ctrl->device_damage, o_ctrl->device_error);
			return -ENODEV;
		}
		...
		if (i2c_list->op_code ==  CAM_SENSOR_I2C_WRITE_RANDOM) {
			...
			if (rc < 0) {
				o_ctrl->device_error++;
				CAM_ERR(CAM_OIS,
					"Failed in Applying i2c wrt settings, IO fail count:%d", o_ctrl->device_error);
				return rc;
			}
			...
			for (i = 0; i < size; i++) {
				...
				if (rc < 0) {
					o_ctrl->device_error++;
					CAM_ERR(CAM_OIS,
						"i2c poll apply setting Fail, IO fail count:%d", o_ctrl->device_error);
					return rc;
		...
		if (i2c_set == NULL) {
			...
			if(!delaytimes){
				...
				if ((i2c_list_last != NULL) && list_entry_is_head(i2c_list, &(i2c_set->list_head), list)) {
					...
					if (false == IS_DEV_IO_ERROR(o_ctrl->device_damage, o_ctrl->device_error)) {
...
static int cam_ois_pkt_parse(struct cam_ois_ctrl_t *o_ctrl, void *arg)
{
	...
	case CAM_OIS_PACKET_OPCODE_READ: {
		...
		if (rc < 0) {
			o_ctrl->device_error++;
			CAM_ERR(CAM_OIS, "cannot read data rc: %d, IO fail count:%d", o_ctrl->device_error);
		...
		if (rc < 0 ||
				true == IS_DEV_IO_ERROR(o_ctrl->device_damage, o_ctrl->device_error)) {
			CAM_ERR(CAM_OIS,
...
int cam_ois_driver_cmd(struct cam_ois_ctrl_t *o_ctrl, void *arg)
{
	...
	case CAM_RELEASE_DEV:{
		...
		if (MAX_IO_FAIL_COUNT <= o_ctrl->device_error ||
				true == o_ctrl->init_error)
		{
			if (MAX_DAMAGE_COUNT > o_ctrl->device_damage)
			{
				o_ctrl->device_damage++;
				CAM_ERR(CAM_OIS, "device is damage count:%d", o_ctrl->device_damage);
			}
		}//If the number of damages has not been reached and no abnormalities have occurred, reset to zero
		else if (MAX_DAMAGE_COUNT > o_ctrl->device_damage){
			o_ctrl->device_damage = 0;
		}
		// xiaomi add end
		break;
	}
