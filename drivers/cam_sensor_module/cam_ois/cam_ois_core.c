	...
	struct i2c_settings_array *i2c_set)
{
	...
	list_for_each_entry(i2c_list,
		...
		if (true == cam_check_cci_is_error(&o_ctrl->cci_err_info)) {
			CAM_ERR(CAM_OIS, "damage count:%d IO fail count:%d skip apply setting!",
				cam_cci_read_damage_count(&o_ctrl->cci_err_info),
				cam_cci_read_io_fail_count(&o_ctrl->cci_err_info));
			return -ENODEV;
		}
		...
		if (i2c_list->op_code ==  CAM_SENSOR_I2C_WRITE_RANDOM) {
			...
			if (rc < 0) {
				cam_cci_add_io_fail_count(&o_ctrl->cci_err_info);
				CAM_ERR(CAM_OIS,
					"Failed in Applying i2c wrt settings, IO fail count:%d",
					cam_cci_read_io_fail_count(&o_ctrl->cci_err_info));
				return rc;
			}
			...
			if (rc < 0) {
				cam_cci_add_io_fail_count(&o_ctrl->cci_err_info);
				CAM_ERR(CAM_OIS,
					"Failed to seq write I2C settings: %d, IO fail count:%d",
					rc, cam_cci_read_io_fail_count(&o_ctrl->cci_err_info));
				return rc;
			}
			...
			for (i = 0; i < size; i++) {
				...
				if (rc < 0) {
					cam_cci_add_io_fail_count(&o_ctrl->cci_err_info);
					CAM_ERR(CAM_OIS,
						"i2c poll apply setting Fail, IO fail count:%d",
						cam_cci_read_io_fail_count(&o_ctrl->cci_err_info));
					return rc;
				} else if (rc ==  I2C_COMPARE_MISMATCH) {
					// xiaomi modify begin
					rc = 0;
					CAM_ERR(CAM_OIS, "%s i2c poll addr 0x%x data 0x%x mismatch",
							o_ctrl->ois_name,
							i2c_list->i2c_settings.reg_setting[i].reg_addr,
							i2c_list->i2c_settings.reg_setting[i].reg_data);
					// xiaomi modify end
		...
		cam_cci_reset_io_fail_count(&o_ctrl->cci_err_info);
	}
...
// xiaomi add start
// ois_apply_settings_control[0] <==> switch
// ois_apply_settings_control[1] <==> real result
// ois_apply_settings_control[2] <==> changed result
static int ois_apply_settings_control_count = 3;
static int ois_apply_settings_control[3] = {0};
module_param_array(ois_apply_settings_control,
					int,
					&ois_apply_settings_control_count,
					0644);
int32_t cam_ois_apply_settings_control_func(
	struct cam_ois_ctrl_t *o_ctrl,
	struct i2c_settings_array *i2c_set)
{
	int32_t rc = 0;
	CAM_DBG(CAM_OIS, "control %s args %d, %d, %d",
			o_ctrl->device_name,
			ois_apply_settings_control[0],
			ois_apply_settings_control[1],
			ois_apply_settings_control[2]);
	if (0 == ois_apply_settings_control[0]) {
		rc = cam_ois_apply_settings(o_ctrl, i2c_set);
	} else if (1 == ois_apply_settings_control[0]) {
		rc = ois_apply_settings_control[2];
		ois_apply_settings_control[1] =
			ois_apply_settings_control[2];
	} else if (2 == ois_apply_settings_control[0]) {
		ois_apply_settings_control[1] =
			cam_ois_apply_settings(o_ctrl, i2c_set);
		rc = ois_apply_settings_control[2];
	}
		...
		if (i2c_set == NULL) {
			...
			if(!delaytimes){
				...
				if ((i2c_list_last != NULL) && list_entry_is_head(i2c_list, &(i2c_set->list_head), list)) {
					...
					if (false == cam_check_cci_is_error(&o_ctrl->cci_err_info)) {
						...
						rc = - ENODEV;
						CAM_ERR(MI_PARKLENS, "damage count:%d IO fail count:%d skip parklens!",
							cam_cci_read_damage_count(&o_ctrl->cci_err_info),
							cam_cci_read_io_fail_count(&o_ctrl->cci_err_info));
					}
...
static int cam_ois_fw_download_v2(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	if (o_ctrl->i2c_fw_version_data.is_settings_valid == 1) {
		...
		rc = cam_ois_apply_settings_control_func(o_ctrl, &o_ctrl->i2c_fw_version_data);
		if ((rc == -EAGAIN) &&
			(o_ctrl->io_master_info.master_type == CCI_MASTER)) {
			CAM_WARN(CAM_OIS,
			"CCI HW is resetting: Reapplying FW init settings");
			usleep_range(1000, 1010);
			rc = cam_ois_apply_settings_control_func(o_ctrl,
...
static int cam_ois_pkt_parse(struct cam_ois_ctrl_t *o_ctrl, void *arg)
{
	...
	case CAM_OIS_PACKET_OPCODE_INIT:
		...
		if (o_ctrl->opcode.customized_ois_flag) {
			for(j = 0; j < sizeof(pflash_ois) / sizeof(struct flash_ois_function) ; j++){
				if(ps[j].flag == o_ctrl->opcode.customized_ois_flag){
					config_flag++;
					if (true == cam_check_cci_is_error(&o_ctrl->cci_err_info)){
						CAM_ERR(CAM_OIS, "damage count:%d IO fail count:%d skip parklens!",
							cam_cci_read_damage_count(&o_ctrl->cci_err_info),
							cam_cci_read_io_fail_count(&o_ctrl->cci_err_info));
						rc = - ENODEV;
						goto pwr_dwn;
		...
		/* xiaomi add end*/
			...
			rc = cam_ois_apply_settings_control_func(o_ctrl, &o_ctrl->i2c_init_data);
			if ((rc == -EAGAIN) &&
				(o_ctrl->io_master_info.master_type == CCI_MASTER)) {
				CAM_WARN(CAM_OIS,
		...
		cam_cci_clear_init_fail(&o_ctrl->cci_err_info);
		break;
	case CAM_OIS_PACKET_OPCODE_OIS_CONTROL:
		...
		rc = cam_ois_apply_settings_control_func(o_ctrl, i2c_reg_settings);
		if (rc < 0) {
	...
	case CAM_OIS_PACKET_OPCODE_READ: {
		...
		if (false == cam_check_cci_is_error(&o_ctrl->cci_err_info)) {
			...
			CAM_ERR(CAM_OIS, "damage count:%d IO fail count:%d skip apply read data!",
				cam_cci_read_damage_count(&o_ctrl->cci_err_info),
				cam_cci_read_io_fail_count(&o_ctrl->cci_err_info));
		}
		...
		if (rc < 0) {
			cam_cci_add_io_fail_count(&o_ctrl->cci_err_info);
			CAM_ERR(CAM_OIS, "cannot read data rc: %d, IO fail count:%d",
				cam_cci_read_io_fail_count(&o_ctrl->cci_err_info));
		...
		if (rc < 0 ||
				true == cam_check_cci_is_error(&o_ctrl->cci_err_info)) {
			CAM_ERR(CAM_OIS,
...
pwr_dwn:
	cam_cci_set_init_fail(&o_ctrl->cci_err_info);
	cam_mem_put_cpu_buf(dev_config.packet_handle);
...
int cam_ois_driver_cmd(struct cam_ois_ctrl_t *o_ctrl, void *arg)
{
	...
	case CAM_ACQUIRE_DEV:
		...
		cam_cci_clear_init_fail(&o_ctrl->cci_err_info);
		cam_cci_reset_io_fail_count(&o_ctrl->cci_err_info);
		cam_check_cci_is_damage(&o_ctrl->cci_err_info, OISI2C);
		o_ctrl->cam_ois_state = CAM_OIS_ACQUIRE;
		break;
	case CAM_START_DEV:
		if (o_ctrl->cam_ois_state != CAM_OIS_CONFIG) {
	...
	case CAM_RELEASE_DEV:{
		...
		if (true == cam_check_cci_is_damage(&o_ctrl->cci_err_info, OISI2C))
			CAM_ERR(CAM_OIS, "dev may damaged!, damage count:%d",
				cam_cci_read_damage_count(&o_ctrl->cci_err_info));
		// xiaomi add end
		break;
	}
