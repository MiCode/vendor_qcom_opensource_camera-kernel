...
int sem1217s_ois_fw_download(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	CAM_INFO(CAM_OIS,
		...
		current_fw_ver, new_fw_ver, o_ctrl->opcode.noUpdateForOisWithFlash);

	if(((((current_fw_ver < new_fw_ver) && (0 == semco_ois_fw_update_op)) ||
		(current_fw_ver != new_fw_ver)) && (FALSE == o_ctrl->opcode.noUpdateForOisWithFlash)) ||
	...
	if (o_ctrl->i2c_init_data.is_settings_valid == 1) {
		...
		if (rc) {
			CAM_ERR(CAM_OIS,"Cannot apply i2c_init_data data %d, flag %d",rc,
				o_ctrl->opcode.customized_ois_flag);
		//	goto pwr_dwn;
		} else {
