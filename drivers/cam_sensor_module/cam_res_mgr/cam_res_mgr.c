...
void led_trigger_event_with_schedul(struct led_trigger *trig,
			enum led_brightness brightness)
{
	...
	led_cdev = list_first_entry(&trig->led_cdevs, typeof(*led_cdev), trig_list);
	if(!list_entry_is_head(list_next_entry(led_cdev, trig_list),
			&trig->led_cdevs, trig_list)) {
		CAM_ERR(CAM_RES, "this function only operation one cdev");
	}
	...
	enum led_brightness brightness)
{
	...
	if (!cam_res) {
		 ...
		 * the led event.
		 */
#if IS_REACHABLE(CONFIG_LEDS_SUBPMIC_FLASH)
		led_trigger_event_with_schedul(trig, brightness);
#else
