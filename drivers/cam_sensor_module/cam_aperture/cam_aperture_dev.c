...
{
	return &g_i3c_aperture_data[index].probe_complete;
}
	...
	unsigned int cmd, void *arg)
{
	...
	case CAM_SD_SHUTDOWN:
		...
		rc = cam_aperture_subdev_close_internal(sd, NULL);
		break;
	default:
		CAM_ERR(CAM_APERTURE, "Invalid ioctl cmd: %u", cmd);
		rc = -ENOIOCTLCMD;
		break;
	}
	return rc;
	...
	unsigned int cmd, unsigned long arg)
{
	...
	case VIDIOC_CAM_CONTROL:
		...
		if (rc) {
			CAM_ERR(CAM_APERTURE,
				"Failed in aperture subdev handling rc: %d",
				rc);
			return rc;
		}
		break;
	...
	struct device *master_dev, void *data)
{
	...
	CAM_INFO(CAM_APERTURE, "i2c remove invoked");
	mutex_lock(&(a_ctrl->aperture_mutex));
	cam_aperture_shutdown(a_ctrl);
	mutex_unlock(&(a_ctrl->aperture_mutex));
	cam_unregister_subdev(&(a_ctrl->v4l2_dev_str));
...
const static struct component_ops cam_aperture_i2c_component_ops = {
	.bind = cam_aperture_i2c_component_bind,
	.unbind = cam_aperture_i2c_component_unbind,
};
	...
	const struct i2c_device_id *id)
{
	...
	CAM_DBG(CAM_APERTURE, "Adding sensor aperture component");
	rc = component_add(&client->dev, &cam_aperture_i2c_component_ops);
	if (rc)
		CAM_ERR(CAM_APERTURE, "failed to add component rc: %d", rc);
	...
	struct device *master_dev, void *data)
{
	...
	CAM_DBG(CAM_APERTURE, "Component bound successfully %d",
	...
	struct device *master_dev, void *data)
{
	...
	devm_kfree(&pdev->dev, a_ctrl);
	CAM_INFO(CAM_APERTURE, "Aperture component unbinded");
}

const static struct component_ops cam_aperture_platform_component_ops = {
	.bind = cam_aperture_platform_component_bind,
	...
	struct platform_device *pdev)
{
	...
	CAM_DBG(CAM_APERTURE, "Adding sensor aperture component");
	rc = component_add(&pdev->dev, &cam_aperture_platform_component_ops);
	if (rc)
		CAM_ERR(CAM_APERTURE, "failed to add component rc: %d", rc);
...
static int cam_aperture_i3c_driver_probe(struct i3c_device *client)
{
	...
	CAM_DBG(CAM_APERTURE, "Probe for I3C Slave %s", dev_name(dev));
	...
	a_ctrl = g_i3c_aperture_data[index].a_ctrl;
	if (!a_ctrl) {
		CAM_ERR(CAM_APERTURE,
	...
	complete_all(&g_i3c_aperture_data[index].probe_complete);

	CAM_DBG(CAM_APERTURE, "I3C Probe Finished for %s", dev_name(dev));
	return rc;
}
...
int cam_aperture_driver_init(void)
{
	...
	if (!dev) {
		CAM_DBG(CAM_APERTURE, "Couldnt Find the i3c-id-table dev node");
		return 0;
	}
...
i3c_register_err:
	i2c_del_driver(&cam_aperture_i2c_driver);
i2c_register_err:
...
void cam_aperture_driver_exit(void)
{
	...
	i3c_driver_unregister(&cam_aperture_i3c_driver);
}
