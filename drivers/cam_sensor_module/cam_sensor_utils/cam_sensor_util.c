	...
	struct cam_sensor_power_ctrl_t *power_info)
{
	...
	for (i = 0; i < count; i++) {
		...
		} else if (!strcmp(seq_name, "wl2866_avdd1")) {
			ps[i].seq_type = SENSOR_WL2866D_AVDD1;
		} else {
		...
		struct cam_hw_soc_info *soc_info)
{
	...
	CAM_DBG(CAM_SENSOR, "Entry! name %s %s %d", soc_info->dev_name, soc_info->label_name, soc_info->index);
	if (!ctrl) {
	...
	for (index = 0; index < ctrl->power_setting_size; index++) {
		...
		CAM_DBG(CAM_SENSOR, "seq_type %d, seq_val %d, config_val %d delay = %d", power_setting->seq_type, power_setting->seq_val, power_setting->config_val,power_setting->delay);
		...
		case SENSOR_CUSTOM_GPIO2:
			...
			CAM_DBG(CAM_SENSOR, "gpio set val %d ,config_val %d",
				gpio_num_info->gpio_num[power_setting->seq_type], power_setting->config_val);
		...
		struct cam_hw_soc_info *soc_info)
{
	...
	for (index = 0; index < ctrl->power_down_setting_size; index++) {
		...
		case SENSOR_WL2866D_AVDD2:
			...
			for(retry = 0; retry < 3; retry++) {
				ret = wl2866d_camera_power_control(wl2866_iotype, WL2866_PWR_DOWN);
				if(ret >= 0)
					break;
			}
			if (ret < 0) {
