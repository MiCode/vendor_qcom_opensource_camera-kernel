	...
	struct cam_sensor_power_ctrl_t *power_info)
{
	...
	for (i = 0; i < count; i++) {
		...
		} else if (!strcmp(seq_name, "cam_custom1")) {
			ps[i].seq_type = SENSOR_CUSTOM_GPIO1;
		} else if (!strcmp(seq_name, "cam_custom2")) {
	...
	for (index = 0; index < ctrl->power_setting_size; index++) {
		...
		case SENSOR_CUSTOM_REG2:
			...
			if (power_setting->seq_val < num_vreg) {
                ...
                if(isIMX852 && !strcmp(soc_info->rgltr_name[vreg_idx], "cam_vana")
                        && (!strcmp(soc_info->dev_name, "ac15000.qcom,cci0:qcom,eeprom2")
                        || !strcmp(soc_info->dev_name, "ac15000.qcom,cci0:qcom,eeprom3")
                        || !strcmp(soc_info->dev_name, "ac15000.qcom,cci0:qcom,eeprom4")))
                {
                    seq_min_volt = 2880000;
                    seq_max_volt = 2880000;
                    CAM_DBG(CAM_SENSOR, "dev_name: %s regulator_name : %s isIMX852 : %d regulator_debug:min_volt %d ,max_volt%d",
                        soc_info->dev_name, soc_info->rgltr_name[vreg_idx], isIMX852, seq_min_volt, seq_max_volt);
                }

				CAM_DBG(CAM_SENSOR, "regulator_debug:min_volt %d ,max_volt%d",
					seq_min_volt, seq_max_volt);
