	...
	void *cb, ktime_t scheduled_time, uint32_t threshold)
{
	...
	if (diff > threshold) {
		...
		// adjust by xiaomi start 
		if (!strcmp(wq_name, "ISP Tasklet") && !strcmp(state, "execution") && diff >= 15 && 
			(ktime_ms_delta(cur_time, last_send_time) > 1000 || init == 0)) {
			last_send_time = cur_time;
			init = 1;
			CAM_INFO(CAM_UTIL,
				"%s cb: %ps delay in %s detected %ld:%06ld cur %ld:%06ld "
				"diff %ld: threshold %d send to pid: %u",
				wq_name, cb, state, scheduled_ts.tv_sec,
				scheduled_ts.tv_nsec/NSEC_PER_USEC,
				cur_ts.tv_sec, cur_ts.tv_nsec/NSEC_PER_USEC,
				diff, threshold, g_CameraPid);
			if (g_CameraPid != 0)
				send_sig(SIGUSR2, find_task_by_vpid(g_CameraPid), 0);
		} else {
			CAM_INFO(CAM_UTIL,
				"%s cb: %ps delay in %s detected %ld:%06ld cur %ld:%06ld "
				"diff %ld: threshold %d",
