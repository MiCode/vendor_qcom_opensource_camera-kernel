...
uint64_t    SOF_frameid[5][2];
static int request_buffer = 0;
...
int cam_debug_record_key_message(int subdev_id,void *ctrl,void *info,uint64_t request_id,int32_t idx,int opcode)
{
	...
	if(subdev_id == CAM_ISP)
	{
		...
		if(opcode == EVENT_ISP_SOF)
		{
			...
			if(soc_info)
			{
				strncpy(cam_key_buffer->event_id[EVENT_ISP_SOF][index].u.frame_info.dev_name,soc_info->dev_name,CAM_SENSOR_NAME_MAX_LENGTH);
			}
			SOF_frameid[soc_info->index][0] = ctx_isp->frame_id;
			...
			SOF_frameid[soc_info->index][1] = req_id;
		}
		else if(opcode == EVENT_ISP_EOF)
		{
			...
			if(soc_info)
			{
				strncpy(cam_key_buffer->event_id[EVENT_ISP_EOF][index].u.frame_info.dev_name,soc_info->dev_name,CAM_SENSOR_NAME_MAX_LENGTH);
			}
		...
		else if(opcode == EVENT_ISP_EPOCH)
		{
			...
			if(soc_info)
			{
				strncpy(cam_key_buffer->event_id[EVENT_ISP_EPOCH][index].u.frame_info.dev_name,soc_info->dev_name,CAM_SENSOR_NAME_MAX_LENGTH);
			}
		...
		else if(opcode == EVENT_ADD_REQ)
		{
			...
			if(soc_info->dev_name)
			{
				strncpy(cam_key_buffer->event_id[EVENT_ADD_REQ][index].u.add_req.dev_name,soc_info->dev_name,CAM_SENSOR_NAME_MAX_LENGTH);
			}
		...
		else if(opcode == EVENT_APPLY_REQ)
		{
			...
			if(soc_info)
			{
				...
				strncpy(cam_key_buffer->event_id[EVENT_APPLY_REQ][index].u.app_req.dev_name,soc_info->dev_name,CAM_SENSOR_NAME_MAX_LENGTH);
			}
	...
	else if(opcode == EVENT_POWER_UP)
	{
		...
		if(soc_info->dev_name)
		{
			strncpy(cam_power_record_buffer->power_seq[0][index].dev_name,soc_info->dev_name,CAM_SENSOR_NAME_MAX_LENGTH);
		}
	...
	else if(opcode == EVENT_POWER_DOWN)
	{
		...
		if(soc_info->dev_name)
		{
			strncpy(cam_power_record_buffer->power_seq[1][index].dev_name,soc_info->dev_name,CAM_SENSOR_NAME_MAX_LENGTH);
		}
	...
	else if(opcode == EVENT_IO)
	{
		if(CAM_SENSOR == subdev_id)
		{
			...
			cam_io_record_buffer->sensor_setting_data[index].frame_id = s_ctrl->current_apply_frame_id;

			cam_io_record_buffer->sensor_setting_data[index].module_id = soc_info->index;
			if(soc_info->dev_name)
			{
				strncpy(cam_io_record_buffer->sensor_setting_data[index].dev_name,soc_info->dev_name,CAM_SENSOR_NAME_MAX_LENGTH);
			}
		...
		else if(CAM_OIS == subdev_id)
		{
			...
			if(soc_info->dev_name)
			{
				strncpy(cam_io_record_buffer->ois_setting[index].dev_name,soc_info->dev_name,CAM_SENSOR_NAME_MAX_LENGTH);
			}
	...
	else if(opcode == EVENT_IO_END)
	{
		if(CAM_SENSOR == subdev_id)
		{
			...
			if(req_id != 0 &&
				req_id == cam_io_record_buffer->sensor_setting_data[index].req_id &&
				s_ctrl->current_apply_frame_id == cam_io_record_buffer->sensor_setting_data[index].frame_id)
			{
				cam_io_record_buffer->event_record_head[1] += request_buffer;
				request_buffer = 0;
			}
			else
			{
				int req_count = 0;
				int next_req  = 0;
				while((req_id != cam_io_record_buffer->sensor_setting_data[index].req_id || s_ctrl->current_apply_frame_id != cam_io_record_buffer->sensor_setting_data[index].frame_id) &&
						req_count < APPLY_SETTING_REQ)
				{
					INC_HEAD(cam_io_record_buffer->event_record_head[1],APPLY_SETTING_REQ,&index);
					req_count++;
					int next_index  = (index + 1) % APPLY_SETTING_REQ;
					int req_0_count = 0;
					next_req        = cam_io_record_buffer->sensor_setting_data[next_index].req_id;

					while(0 == cam_io_record_buffer->sensor_setting_data[index].req_id && next_req == 0 &&
							req_0_count < APPLY_SETTING_REQ)
					{
						INC_HEAD(cam_io_record_buffer->event_record_head[1],APPLY_SETTING_REQ,&index);
						next_index = (index + 1) % APPLY_SETTING_REQ;
						next_req = cam_io_record_buffer->sensor_setting_data[next_index].req_id;
						req_0_count++;
					}
					if(req_0_count != 0)
					{
						req_count += req_0_count;
					}
				}
				request_buffer = req_count;
				cam_io_record_buffer->event_record_head[1] -= request_buffer + 1;
			}
			...
			uint64_t frame_id = s_ctrl->current_apply_frame_id;
			uint64_t op = idx;

			if(op == CAM_SENSOR_PACKET_OPCODE_SENSOR_DEFERRED_META)
			{
				frame_id += 1;
			}

			if( frame_id != 0 && frame_id != -1 && req_id != 0 && 
				SOF_frameid[cam_io_record_buffer->sensor_setting_data[index].module_id][1] != 0 &&
				frame_id < SOF_frameid[cam_io_record_buffer->sensor_setting_data[index].module_id][0] &&
				SOF_frameid[cam_io_record_buffer->sensor_setting_data[index].module_id][1] >= req_id -1)
				{
					CAM_DBG(CAM_UTIL,"succes get in DUMP_TOOL_TYPE_APPLY_DELAY,module_id:%d,current apply id:%d,last_frameid:%d,current_req:%d,current_module_id:%d",cam_io_record_buffer->sensor_setting_data[index].module_id,frame_id,SOF_frameid[cam_io_record_buffer->sensor_setting_data[index].module_id][0],req_id,cam_io_record_buffer->sensor_setting_data[index].module_id);

					// struct cam_req_mgr_message  req_msg = {0};
					// req_msg.u.frame_msg.request_id = req_id;
					// req_msg.u.frame_msg.frame_id = frame_id;

					// cam_req_mgr_notify_message(&req_msg,
					// 							V4L_EVENT_DUMP_TOOL_APPLY_DELAY,
					// 							V4L_EVENT_DUMP_TOOL_EVENT);
				}
