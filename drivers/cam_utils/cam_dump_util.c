...
int cam_debug_record_key_message(int subdev_id,void *ctrl,void *info,uint64_t request_id,int32_t idx,int opcode)
{
	...
	if(subdev_id == CAM_ISP)
	{
		...
		else if(opcode == EVENT_ADD_REQ)
		{
			...
			{
				strlcpy(cam_key_buffer->event_id[EVENT_ADD_REQ][index].u.add_req.dev_name,soc_info->dev_name,CAM_SENSOR_NAME_MAX_LENGTH);
			}
		...
		else if(opcode == EVENT_APPLY_REQ)
		{
			...
			if(soc_info)
			{
				...
				strlcpy(cam_key_buffer->event_id[EVENT_APPLY_REQ][index].u.app_req.dev_name,soc_info->dev_name,CAM_SENSOR_NAME_MAX_LENGTH);
			}
	...
	else if(opcode == EVENT_POWER_UP)
	{
		...
		if(soc_info->dev_name)
		{
			strlcpy(cam_power_record_buffer->power_seq[0][index].dev_name,soc_info->dev_name,CAM_SENSOR_NAME_MAX_LENGTH);
		}
	...
	else if(opcode == EVENT_POWER_DOWN)
	{
		...
		if(soc_info->dev_name)
		{
			strlcpy(cam_power_record_buffer->power_seq[1][index].dev_name,soc_info->dev_name,CAM_SENSOR_NAME_MAX_LENGTH);
		}
	...
	else if(opcode == EVENT_IO)
	{
		if(CAM_SENSOR == subdev_id)
		{
			...
			if(soc_info->dev_name)
			{
				strlcpy(cam_io_record_buffer->sensor_setting_data[index].dev_name,soc_info->dev_name,CAM_SENSOR_NAME_MAX_LENGTH);
			}
		...
		else if(CAM_OIS == subdev_id)
		{
			...
			if(soc_info->dev_name)
			{
				strlcpy(cam_io_record_buffer->ois_setting[index].dev_name,soc_info->dev_name,CAM_SENSOR_NAME_MAX_LENGTH);
			}
