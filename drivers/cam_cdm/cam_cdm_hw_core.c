...
int32_t g_rst_flag = 0;
uint32_t g_irq_cnt = 0;
int32_t g_rst_flag_deinit = 0;
uint32_t g_irq_cnt_deinit = 0;
...
int cam_hw_cdm_reset_hw(struct cam_hw_info *cdm_hw, uint32_t handle)
{
	...
	if (time_left <= 0) {
		...
		CAM_ERR(CAM_CDM, "%s%u HW reset Wait failed rc=%d",
			soc_info->label_name,
			soc_info->index, rc);
		CAM_INFO(CAM_CDM,"after rst timeout cdm irq cnt: %d",g_irq_cnt);
	...
	void *init_hw_args, uint32_t arg_size)
{
	...
	if (time_left <= 0) {
		...
		CAM_INFO(CAM_CDM,"after rst timeout cdm irq cnt: %d",g_irq_cnt_deinit);
		cam_hw_cdm_dump_core_debug_registers(cdm_hw, false);
	}
	g_rst_flag_deinit = 0;
	g_irq_cnt_deinit = 0;
