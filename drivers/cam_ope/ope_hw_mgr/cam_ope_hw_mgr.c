...
static int cam_ope_mgr_acquire_hw(void *hw_priv, void *hw_acquire_args)
{
	...
	cam_ctx = (struct cam_context *)args->context_data;
	...
	if (rc == -ETIMEDOUT) {
		CAM_ERR(CAM_OPE, "cdm stream on failure: %d, Triggering Full recovery", rc);
		cam_context_util_cdm_recovery(cam_ctx);
	...
	struct cam_hw_flush_args *flush_args)
{
	...
	cam_ctx = (struct cam_context *)flush_args->context_data;

	rc = cam_cdm_flush_hw(ctx_data->ope_cdm.cdm_handle);
	if (rc == -ETIMEDOUT) {
		CAM_ERR(CAM_OPE,"Flush failed due to reset timeout, trigger Full recovery");
		cam_context_util_cdm_recovery(cam_ctx);
		return 0;
	}
