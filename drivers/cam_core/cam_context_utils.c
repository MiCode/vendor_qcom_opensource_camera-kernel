...
void cam_context_util_cdm_recovery(struct cam_context *cam_ctx)
{
	struct cam_req_mgr_message      v4l2_msg = {0};

	v4l2_msg.u.err_msg.device_hdl  = cam_ctx->dev_hdl;
	v4l2_msg.u.err_msg.error_type  = CAM_REQ_MGR_ERROR_TYPE_FULL_RECOVERY;
	v4l2_msg.u.err_msg.link_hdl    = cam_ctx->link_hdl;
	v4l2_msg.u.err_msg.request_id  = -1;
	v4l2_msg.session_hdl           = cam_ctx->session_hdl;

	CAM_ERR(CAM_CRM,
		"Sending Error Event with dev hdl = 0x%x link hdl =0x%x session_hdl = 0x%x",
		cam_ctx->dev_hdl, cam_ctx->link_hdl, cam_ctx->session_hdl);
	cam_req_mgr_notify_message(&v4l2_msg, V4L_EVENT_CAM_REQ_MGR_ERROR,
