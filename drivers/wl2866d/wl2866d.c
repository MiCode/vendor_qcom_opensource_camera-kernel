...
static void enable_i2c_pullup()
{
    ...
    if (!gpio_is_valid(iov_gpio))
    {
        ...
        else
        {
            ...
            pr_info("wl2866d:iov_gpio set high, free it.\n");
...
static int wl2866d_power_on(struct device *dev)
{
    ...
    if (ret < 0)
    {
		...
		pr_info("wl2866d:en_gpio set low\n");
    }
...
static int wl2866d_init_register(void)
{
    ...
    for(i = 0; i < 16; i++){
        for (j = 0; j < 3; j++)
        {
            ...
            if(rc) {
                pr_err("%s write 0x%x 0x%x failed, to retry %d\n", __func__, reg[i], val[i], j);
                if (2 == j)
                {
                    return -1;
                }
...
static int wl2866d_match_id(struct device *dev)
{
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        if (ret < 0 ||
            ...
            pr_err("wl2866d: is not found %d %x, to retry %d\n", ret, wl2866d_data.chip_id, i);
            if (2 == i)
            {
                return -ENODEV;
            }
...
int wl2866d_camera_power_control(unsigned int out_iotype, int is_power_on)
{
    ...
    unsigned char DVDD_value = 0, AVDD_value = 0;
    ...
    if(is_power_on) {
...
#endif
        if (600000 <= is_power_on && 1800000 >= is_power_on && 
            (OUT_DVDD1 == out_iotype || OUT_DVDD2 == out_iotype))
        {
            pr_info("wl2866d pwr_ctrl: power on, set DVDD voltage %d\n", is_power_on);
            DVDD_value = (is_power_on - 600000) / 6000;
            ret = wl2866d_read_reg(wl2866d_on_config[out_iotype].reg, &reg_read);
            if(ret < 0 || reg_read != DVDD_value){
                ret = wl2866d_write_reg(wl2866d_on_config[out_iotype].reg, DVDD_value);
                if (ret < 0 )
                {
                ...
                pr_info("wl2866d: voltage is right, no need write reg\n");
            }
        }
        else if (1200000 <= is_power_on && 4300000 >= is_power_on && 
                 (OUT_AVDD1 == out_iotype || OUT_AVDD2 == out_iotype))
        {
            pr_info("wl2866d pwr_ctrl: power on, set AVDD voltage %d\n", is_power_on);
            AVDD_value = (is_power_on - 1200000) / 12500;
            ret = wl2866d_read_reg(wl2866d_on_config[out_iotype].reg, &reg_read);
            if(ret < 0 || reg_read != AVDD_value){
                ret = wl2866d_write_reg(wl2866d_on_config[out_iotype].reg, AVDD_value);
                if (ret < 0 )
                {
                    pr_err("wl2866d: set voltage fail %d\n", out_iotype);
                    goto fail_return;
        ...
        {
            pr_info("wl2866d pwr_ctrl: power on, set voltage %d\n",
	    ...
	    pr_info("wl2866d pwr_ctrl: power on, set reg %02x\n", reg_val);
    }
    else{
	    reg_val &= ~(1 << out_iotype);
	    pr_info("wl2866d pwr_ctrl: power off, set reg %02x\n", reg_val);
    }
...
int cam_wl2866_init_module(void)
{
    ...
    do{
        ...
        if (ret == 0){
            ...
            pr_err("%s: add wl2866d driver failed, error=%d, retry times=%d\n",
                __func__, ret, num_retry);
            num_retry--;
        }
