...
int wl2866d_camera_power_control(unsigned int out_iotype, int is_power_on)
{
    ...
    if(is_power_on) {
        ...
        if ((1100000 == is_power_on) &&  (OUT_DVDD2 == out_iotype)) {
			ret = wl2866d_write_reg(wl2866d_on_config[out_iotype].reg, 0x54);
            if (ret < 0)
                printk("hzk wl2866d pwr_ctrl set DVDD2 voltage failed\n");
            else
                printk("hzk set DVDD2 to 1.1V\n");
