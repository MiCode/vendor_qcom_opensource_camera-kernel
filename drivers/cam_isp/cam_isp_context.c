	...
	struct cam_isp_context *ctx_isp, void *evt_data)
{
	...
	ctx_isp->eof_in_applied = false; //add by xiaomi
	...
	struct cam_isp_context *ctx_isp, void *evt_data)
{
	...
	if (rc)
		CAM_ERR(CAM_ISP, "ctx:%u link: 0x%x Failed to get timestamp from HW",
			ctx->ctx_id, ctx->link_hdl);
	/* add by xiaomi to avoid frame_drop */
	if (ctx_isp->substate_activated == CAM_ISP_CTX_ACTIVATED_APPLIED) {
		ctx_isp->eof_in_applied = true;
	}
	/* end xiaomi*/
	//xiaomi add
		...
		if (req_isp->num_fence_map_out == req_isp->num_acked)
			...
			CAM_ERR(CAM_ISP,
				"receive rup in unexpected state, ctx_idx: %u, link: 0x%x",
				 ctx->ctx_id, ctx->link_hdl);
// xiaomi add to fix reg update lose
	} else {
		atomic_set(&ctx_isp->unserved_rup, 1);
	...
	void *evt_data)
{
	...
	if (ctx_isp->bubble_recover_dis && !ctx_isp->sfe_en) {
		//adjust by xiaomi add (&& ctx_isp->eof_in_applied) to avoid frame drop
		if (atomic_read(&ctx_isp->unserved_rup) && ctx_isp->eof_in_applied) {
		//end xiaomi
	...
	struct cam_context *ctx, struct cam_req_mgr_apply_request *apply)
{
	...
	/* xiaomi add support skip first frame for EPCR begin */
	if ((isp_ctx->frame_id == 1) && (!list_empty(&ctx->pending_req_list)) &&
		isp_ctx->epcr_mode && !isp_ctx->vfps_aux_context) {
		struct cam_ctx_request *req;
		...
		apply->request_id = req->request_id;
		__cam_isp_ctx_apply_req_in_activated_state(ctx, apply, CAM_ISP_CTX_ACTIVATED_APPLIED);
	/* xiaomi add support skip first frame for EPCR end */
	} else if (isp_ctx->use_default_apply) {
	...
	void *cmd)
{
	...
	ctx_isp->epcr_mode = false; // xiaomi add support skip first frame for EPCR
	ctx_isp->epcr_skip_fisrt_frame = false; //xiaomi add support skip first frame for EPCR
	...
	struct cam_context *ctx, struct cam_config_dev_cmd *cmd)
{
	...
	bool                             skip_add_req = false; // xiaomi add support skip first frame for EPCR begin
	...
	/* xiaomi add support skip first frame for EPCR begin */
	if ((packet->header.request_id && (((packet->header.op_code + 1) & 0xF) == CAM_ISP_PACKET_INIT_DEV)) &&
		!ctx_isp->vfps_aux_context && ctx_isp->epcr_skip_fisrt_frame) {
		packet->header.op_code += 1;
		req_isp->hw_update_data.packet_opcode_type = CAM_ISP_PACKET_UPDATE_DEV;
		skip_add_req = true;
		ctx_isp->epcr_mode = true;
	}
	/* xiaomi add support skip first frame for EPCR end */
	...
	if (req_isp->hw_update_data.packet_opcode_type ==
		...
		if ((ctx->state < CAM_CTX_READY) && (!ctx_isp->epcr_mode)) { //xiaomi add support skip first frame for EPCR
			rc = -EINVAL;
			CAM_ERR(CAM_ISP,
		...
		if ((ctx_isp->offline_context) || (ctx_isp->vfps_aux_context)) {
			...
			if (ctx->ctx_crm_intf && ctx->ctx_crm_intf->add_req && !skip_add_req) { //xiaomi add support skip first frame for EPCR
	...
	struct cam_req_mgr_link_evt_data *link_evt_data)
{
	...
	case CAM_REQ_MGR_LINK_EVT_UPDATE_PROPERTIES:
		...
		/* xiaomi add support skip first frame for EPCR begin */
		if (link_evt_data->u.properties_mask &
			CAM_LINK_PROPERTY_SENSOR_EPCR_SKIP_FIRST_FRAME)
			ctx_isp->epcr_skip_fisrt_frame = true;
		else
			ctx_isp->epcr_skip_fisrt_frame = false;
		CAM_DBG(CAM_ISP, "epcr_skip_fisrt_frame:%s on ctx: %u link: 0x%x",
			CAM_BOOL_TO_YESNO(ctx_isp->epcr_skip_fisrt_frame), ctx->ctx_id, ctx->link_hdl);
		/* xiaomi add support skip first frame for EPCR end */
		break;
