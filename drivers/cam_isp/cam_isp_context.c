...
free_req:
	...
	CAM_INFO(CAM_ISP, "free_req: handle=%d size=%u cpu_addr=(%s)",
	kmd_cmd_buff_info->handle, kmd_cmd_buff_info->size,
	kmd_cmd_buff_info->cpu_addr ? "NOT NULL" : "NULL");
	if (kmd_cmd_buff_info->cpu_addr)
		cam_mem_put_kref(kmd_cmd_buff_info->handle);
free_req_pkt:
	spin_lock_bh(&ctx->lock);
	list_add_tail(&req->list, &ctx->free_req_list);
	spin_unlock_bh(&ctx->lock);
	CAM_INFO(CAM_ISP, "free_req_pkt: ctx_id=%d req_id=%lld", ctx->ctx_id, req->request_id);
	cam_mem_put_cpu_buf((int32_t) cmd->packet_handle);
	return rc;
}
