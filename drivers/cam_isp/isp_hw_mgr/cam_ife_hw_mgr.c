...
/* xiaomi increase internal recovery count */
#define MAX_INTERNAL_RECOVERY_ATTEMPTS    10
/* xiaomi increase internal recovery count */
	...
	if (cfg->num_hw_update_entries > 0) {
		...
		if (wait_for_cdm) {
			rem_jiffies = cam_common_wait_for_completion_timeout(
				...
				msecs_to_jiffies(300)); //xiaomi change this from 60 to 300
			if (rem_jiffies == 0) {
				CAM_ERR(CAM_ISP,
...
static int cam_ife_mgr_stop_hw(void *hw_mgr_priv, void *stop_hw_args)
{
	...
	if (!atomic_read(&ctx->cdm_done)) {
		rem_jiffies = cam_common_wait_for_completion_timeout(
				&ctx->config_done_complete,
				msecs_to_jiffies(50));  //xiaomi change this from 10 to 50
	...
	struct cam_hw_cmd_args *hw_cmd_args)
{
	...
	/* workaround avoid page fault leads to crash */
	return;
...
static int cam_ife_mgr_cmd(void *hw_mgr_priv, void *cmd_args)
{
	...
	case CAM_HW_MGR_CMD_REG_DUMP_ON_FLUSH:
		...
		if (!atomic_read(&ctx->cdm_done)) {
			rem_jiffies = cam_common_wait_for_completion_timeout(
					&ctx->config_done_complete, msecs_to_jiffies(150));  //xiaomi change this from 30 to 150
			if (rem_jiffies == 0)
				CAM_ERR(CAM_ISP,
