...
static int mipi_error_cnt = 6;
static int mipi_error[6] = {0, 0, 0, 0, 0, 0};
...
module_param_array(mipi_error, int, &mipi_error_cnt, 0644);
...
void count_mipi_error(uint32_t res_type)
{
	...
	if (res_type < mipi_error_flag_cnt + 0x4001 && res_type >= 0x4001)
	{
		...
		mipi_error[phy_id] = mipi_error[phy_id] % 65535;
		CAM_ERR(CAM_ISP, "set mipi error flag: phy_id: %d, timestamp: sec: %ld, nsec: %ld, crc_error_cnt:%d, mipi_error_cnt:%d",
				phy_id, timestamp.tv_sec, timestamp.tv_nsec, crc_error_cnt, mipi_error[phy_id]);
	}
}
