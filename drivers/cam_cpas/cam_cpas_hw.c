	...
	struct cam_cpas_private_soc *soc_private, struct cam_cpas *cpas_core, int cesta_drv_idx)
{
	...
	if (cpas_core->streamon_clients) {
		...
		rc = cam_soc_util_get_clk_level(soc_info, drv_high_clk_rate,
			soc_info->src_clk_idx, &clk_lvl);
		if (rc) {
			CAM_ERR(CAM_CPAS, "failed to get clk lvl: %d, src clk idx: %d, drv clk rate %lld",
				rc, soc_info->src_clk_idx, drv_high_clk_rate);
			return rc;
		}

		cpas_core->drv_axi_bw_calc_lvl = clk_lvl;
	...
	struct cam_hw_soc_info *soc_info)
{
	...
	CAM_DBG(CAM_CPAS,
		"Finding max of hlos axi floor lvl: %d, hlos axi lvl: %d, drv axi lvl: %d",
		cpas_core->hlos_axi_floor_lvl, cpas_core->hlos_axi_bw_calc_lvl,
		cpas_core->drv_axi_bw_calc_lvl);

	highest_level = max(cpas_core->hlos_axi_floor_lvl, cpas_core->hlos_axi_bw_calc_lvl);
	highest_level = max(highest_level, cpas_core->drv_axi_bw_calc_lvl);
