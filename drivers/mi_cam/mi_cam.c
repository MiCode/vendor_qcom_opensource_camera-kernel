...
void map_cluster_cpu(void)
{
	...
	for(k = 0; k <MAXCLUSTER; k++)
	{
		if(0==k) {
			...
			cpu_map[k].max_index = 5;
		}
		if(1==k) {
			cpu_map[k].min_index = 6;
...
static inline void read_event(struct event_data *event, unsigned int cpu)
{
	...
	int ret;
	u64 total;
	unsigned int local_cpu = get_cpu();
	put_cpu();
...
static unsigned int map_cpubg_scale_freq(unsigned long max_freq, unsigned long freq)
{
	unsigned long max_freq_lvl = min(max_freq, cam_cpubg_max_freq);

	/*
	 *
	unsigned long scaled_freq = 0;
	if (util > cam_cpubg_target_load_thresh)
	{
		scaled_freq = max(
			(max_freq + (max_freq >> 4)) * util,
			(max_freq + (max_freq >> 2)) * cam_cpubg_target_load_thresh)/1024;
		return min(min(scaled_freq, freq), max_freq_lvl);
	} else
	*/
	return min(freq, max_freq_lvl);
}
...
static unsigned int map_coeff_freq(unsigned int cpu, unsigned long max_freq, unsigned long util, unsigned long max )
{
	...
	if(cpu_param) {
		pr_info("%s: [MICAM_SCHED]  map_coeff_freq[%u]: target_freq[%lu], util = %lu, max = %lu, new %llu", __func__,cpu, target_freq, util, max, new);
	}
	...
	unsigned long *next_freq, struct cpufreq_policy *policy, bool *need_freq_update)
{
	...
	if(cpu_param == 2) {
		pr_info("%s:  [MICAM_SCHED] cpu = %u, max = %lu, walt_util = %lu, walt_freq = %lu, exc_util = %u, enable %u",
	...
	} else if (cam_cpubg_pressure_enable && cpu <= 2) {
		*next_freq = map_cpubg_scale_freq(policy->cpuinfo.max_freq, old_freq);
		if (*next_freq) {
			if (*next_freq != old_freq) {
				*need_freq_update = true;
			}
		}
		trace_cam_set_next_freq(enable_feature, cam_cpubg_pressure_enable, cpu, policy->cpuinfo.max_freq, util,
			freq, old_freq, *next_freq, *need_freq_update);
	}
