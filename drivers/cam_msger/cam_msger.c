...
static ssize_t cameramsger_read(struct file *fp, char __user *buff,  size_t length, loff_t *ppos) {
    ...
    switch_enable_and_exec(COC_ENABLE){
        ...
        if(idx < READER_COUNT_MAX) {
            ...
            m_pr(LTYPE_COC, "cameramsger_read message(msg-idx:%d) read finish, mssage:%s, len:%d, ret:%d, message_len:%d\n"
                 , reader->front, &(reader->msgs[reader->front]).data
...
static ssize_t cameramsger_write(struct file *fp, const char *buff,  size_t length, loff_t *ppos){
    ...
    m_pr(LTYPE_COC, "%s len:%d", __func__, length);
    ...
    m_pr(LTYPE_COC, "%s: copy after:%s len:%d cpy_len:%d",__func__, &(c_msger_dev.d_cache[0]), length, cpy_len);
...
static int __init cameramsger_init(void)
{
    ...
    c_msger_dev.cammsger_class =
        class_create(THIS_MODULE, c_msger_dev.name);
