...
#define WRITE_ERROR_MEM_ALLOC_FAILED -1019
...
unsigned int msger_siwtches = COC_ENABLE | DPM_ENABLE | MM_ENABLE;
static struct cammsger_dev c_msger_dev;
static struct submodule submodules[] = {
	//{init, write, read, pool, ioctl, release}
	{NULL, get_tgid_by_cmdline, NULL, NULL, NULL, NULL},//cam_pid.h
	{cam_mm_init, cam_mm_rw, NULL, NULL, cam_mm_ioctl, NULL}, //cam_mm.h
...
static void clear_r_entity(int idx)
{
	...
	memset(&(c_msger_dev.readers[idx].msgs), 0, sizeof(struct m_item) * MSG_COUNT_MAX);
...
static int find_reader_by_name_or_tgid(char *name, int tgid)
{
	...
	for (idx = 0; idx < READER_COUNT_MAX; idx++) {
		...
		if (name != NULL && strcmp(name, c_msger_dev.readers[idx].name) == 0)
			return idx;
...
{
	m_pr(LTYPE_COC, "%s, thread:%d file:%p\n", __func__, current->pid, filp);
	return 0;
}
static unsigned int cameramsger_poll(struct file *file, struct poll_table_struct *poll_table)
{
	...
	if (msger_siwtches & COC_ENABLE) {
		m_pr(LTYPE_COC, "%s pid:%d tid:%d file:%p\n", __func__, current->tgid, current->pid, file);
		mutex_lock(&c_msger_dev.g_lock);
		for (idx = 0; idx < READER_COUNT_MAX; idx++) {
			if (!c_msger_dev.readers[idx].used)
				continue;
		...
		if (idx < READER_COUNT_MAX) {//found reader
			if (c_msger_dev.readers[idx].front != c_msger_dev.readers[idx].rear) { //have messages
				...
				m_pr(LTYPE_COC, "%s tid:%d have messages.\n", __func__, current->pid);
				return mask;
			}
			m_pr(LTYPE_COC, "%s poll_wait_begin.\n", __func__);
			poll_wait(file, &(c_msger_dev.readers[idx].wait_queue), poll_table);
			m_pr(LTYPE_COC, "%s poll_wait_end.\n", __func__);
			if (c_msger_dev.readers[idx].front == c_msger_dev.readers[idx].rear) {
				mask = 0;
				return mask;//message array empty
			}
			mask = POLLIN | POLLRDNORM;
			m_pr(LTYPE_COC, "%s reader pid:%d tid:%d fp:%p have messages.\n", __func__, current->pid, current->tgid, file);
			return mask;
		}
	}
	//check submodules
	...
	if (msger_siwtches & COC_ENABLE) {
		...
		if (idx < READER_COUNT_MAX) {
			if (reader->front == reader->rear) {
				...
				mutex_unlock(&c_msger_dev.g_lock);
				return ret;//message array empty
			}

			m_pr(LTYPE_COC, "%s find message curr-tid:%d c_msger_dev.readers[idx]_info pid:%d file:%p.\n",
							__func__, current->pid, reader->tgid, reader->fp);
			...
			m_pr(LTYPE_COC, "%s message(msg-idx:%d) read finish, mssage:%s, len:%d, ret:%d, message_len:%lu\n", __func__
...
static int add_message(int reader_idx, struct m_item *item)
{
	int  rear = 0;
	struct r_entity *reader = NULL;
	...
	memcpy(&(reader->msgs[reader->rear]), item, sizeof(struct m_item));
	reader->rear = rear;
...
static int snd_message(char *buff, int length)
{
	...
	item    = kmalloc(sizeof(struct m_item), GFP_KERNEL);
	sender  = kmalloc_array(READER_ID_MAX, sizeof(char), GFP_KERNEL);
	recever = kmalloc_array(READER_ID_MAX, sizeof(char), GFP_KERNEL);

	if (!sender || !recever) {
		m_pr(LTYPE_COC, "%s sender/recever kmalloc failed!\n", __func__);
		ret = WRITE_ERROR_MEM_ALLOC_FAILED;
		goto __free_mem_and_ret_;
	}

	memset(sender,  0, READER_ID_MAX);
	memset(recever, 0, READER_ID_MAX);
	memset(item,   0, sizeof(struct m_item));
	...
	if (at_pos[1] - at_pos[0] <= 1 || at_pos[2] - at_pos[1] <= 1) {
		ret = WRITE_ERROR_FORMAT_INVALID;
		goto __free_mem_and_ret_;
	}
...
__free_mem_and_ret_:
	if (sender) {
		m_pr(LTYPE_COC, "%s free sender:%p mem\n", __func__, sender);
		kfree(sender);
	}
	if (recever) {
		m_pr(LTYPE_COC, "%s free recever:%p mem\n", __func__, recever);
		kfree(recever);
	}
	if (item) {
		m_pr(LTYPE_COC, "%s free m_item:%p mem\n", __func__, item);
		kfree(item);
	}
	return ret;
}
...
static ssize_t cameramsger_write(struct file *fp, const char *buff,  size_t length, loff_t *ppos)
{
	...
	m_pr(LTYPE_COC, "%s: kouguisen copy after:%s len:%zu cpy_len:%d", __func__, &(c_msger_dev.d_cache[0]), length, cpy_len);
...
static int cameramsger_release(struct inode *inode, struct file *filp)
{
	...
	if (msger_siwtches & COC_ENABLE) {
		...
		if (reader_idx && reader_idx < READER_COUNT_MAX) {
			m_pr(LTYPE_COC, "%s find reader:pid:%d tid:%d fp:%p name:%s:\n", __func__, c_msger_dev.readers[reader_idx].tgid
			...
			if (rl_task == NULL) {
				...
				m_pr(LTYPE_COC, "[%s] pid_alive:%d\n", __func__, pid_alive(rl_task));
				if (!is_rl_task_die && (rl_task->__state == TASK_DEAD || rl_task->exit_state == EXIT_DEAD || rl_task->exit_state == EXIT_ZOMBIE || rl_task->files == NULL))
					is_rl_task_die = true;
			...
			if (is_rl_task_die) {
				for (idx = 0; idx < READER_COUNT_MAX; idx++) {
					if (!c_msger_dev.readers[idx].used || idx == reader_idx)
						continue;
...
static long cameramsger_ioctl(struct file *filp, unsigned int cmd,  unsigned long arg)
{
	...
	switch (cmd) {
	case IOC_CMD_DPM_DEBUG_ON:
		mutex_lock(&c_msger_dev.g_lock);
		c_msger_dev.debug = LTYPE_DPM;
		mutex_unlock(&c_msger_dev.g_lock);
		return 0;
	case IOC_CMD_MM_DEBUG_ON:
		mutex_lock(&c_msger_dev.g_lock);
		c_msger_dev.debug = LTYPE_MM;
		mutex_unlock(&c_msger_dev.g_lock);
		return 0;
...
static const struct file_operations dev_fops = {
	.owner =	THIS_MODULE,
	.open  = cameramsger_open,
	.poll  = cameramsger_poll,
	.read  = cameramsger_read,
	.write = cameramsger_write,
	.release = cameramsger_release,
	.unlocked_ioctl = cameramsger_ioctl,
};
...
static int __init cameramsger_init(void)
{
	...
	m_pr(LTYPE_COC, "mode init begin\n");
	strscpy(c_msger_dev.name, "cam_msger", 10);
	strscpy(c_msger_dev.version, "2.0", 4);

	c_msger_dev.cammsger_major =
		register_chrdev(0, c_msger_dev.name, &dev_fops);
